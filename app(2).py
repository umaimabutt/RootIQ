...
def load_data(uploaded_file) -> Tuple[pd.DataFrame, str]:
    if uploaded_file is None:
        raise DataLoadError("Please upload a CSV, Excel, or JSON file.")
    ...
    try:
        if name.endswith(".csv"):
            ...
            raise DataLoadError("PDF is not supported in this MVP. Please use CSV, XLSX, or JSON.")
        else:
            raise DataLoadError("Unsupported file type. Use CSV, XLSX, or JSON.")
    ...
    # Clean duplicate column names without losing data.
    df = df.copy()
...
def gemini_call(system_prompt: str, payload_json: str) -> str:
    if genai is None:
        raise AIProviderError("Google GenAI SDK is not installed. Run pip install -r requirements.txt.")
    key = get_secret("GEMINI_API_KEY")
    if not key:
        raise AIProviderError("GEMINI_API_KEY is missing. Add it to Streamlit Secrets or your environment.")
    try:
        client = genai.Client(api_key=key)
        response = client.models.generate_content(
            model=GEMINI_MODEL,
            contents=payload_json,
            config={"system_instruction": system_prompt, "temperature": 0.2, "max_output_tokens": 6000},
        )
        return response.text
    except Exception as exc:
        raise AIProviderError(f"Gemini request failed: {exc}")
...
def parse_ai_json(raw_text: str) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
    ...
    if not isinstance(data, dict) or any(k not in data for k in REQUIRED_AI_KEYS):
        return None, raw_text

    for key in REQUIRED_AI_KEYS:
        if key in {"key_findings", "additional_data_needed", "next_actions"} and not isinstance(data[key], list):
            return None, raw_text
    return data, None


def analyze_with_ai(payload: Dict[str, Any], provider: str = "Groq") -> Tuple[Optional[Dict[str, Any]], Optional[str], Optional[str]]:
    payload_json = json.dumps(payload, ensure_ascii=False, default=str)
    raw = groq_call(SYSTEM_PROMPT, payload_json) if provider == "Groq" else gemini_call(SYSTEM_PROMPT, payload_json)
...
def chat_with_dataset(question, payload, provider, history):
    prompt = build_chat_payload(question, payload, history)
    if provider == "Groq":
        return groq_chat_text(prompt)
    return gemini_chat_text(prompt)
...
def gemini_chat_text(prompt):
    if genai is None:
        raise AIProviderError("Google GenAI SDK is not installed.")
    key = get_secret("GEMINI_API_KEY")
    if not key:
    ...
    response = client.models.generate_content(
        model=GEMINI_MODEL,
        contents=prompt,
        config={"system_instruction": "You are RootIQ, a document-only business data assistant.", "temperature": 0.2, "max_output_tokens": 2500},
    )
    return response.text
...
def main():
    ...
    if analyze:
        ...
        if uploaded is None:
            st.error("Upload a CSV, XLSX, or JSON file first.")
            return
        ...
        with st.spinner("Loading, profiling, and analyzing your data..."):
            try:
                ...
                ai_error = None
                try:
                    ai, raw_ai, _ = analyze_with_ai(payload, provider)
